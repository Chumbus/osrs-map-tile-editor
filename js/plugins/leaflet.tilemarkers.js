...
(function (factory) {
    ...
    function cssToArgb(cssColor, opacity) {
        ...
        let a = Math.round((opacity != null ? opacity : 1) * 255);
    ...
    function packMarkers(markers) {
        ...
        for (let m of markers) {
            let labelBytes = m.label ? encoder.encode(m.label) : new Uint8Array(0);
            let labelLen = Math.min(labelBytes.length, 255);
            let chunk = new Uint8Array(10 + labelLen);
            let view = new DataView(chunk.buffer);
            view.setUint16(0, m.regionId);
            chunk[2] = m.regionX;
            chunk[3] = m.regionY;
            chunk[4] = m.z;
            let colorInt = parseInt(m.color.replace('#', ''), 16) >>> 0;
        ...
        let offset = 0;
        for (let p of parts) { result.set(p, offset); offset += p.length; }
        return result;
    }
    ...
    function unpackMarkers(bytes) {
        ...
        while (off + 10 <= bytes.length) {
            let regionId = view.getUint16(off);
            let regionX = bytes[off + 2];
            let regionY = bytes[off + 3];
            let z = bytes[off + 4];
            let colorInt = view.getUint32(off + 5) >>> 0;
            let labelLen = bytes[off + 9];
            let label = undefined;
    ...
    L.Control.Display.TileMarkers = L.Control.Display.extend({
        ...
        createInterface: function () {
            let container = L.DomUtil.create('div', 'leaflet-control-display-expanded tilemarker-panel');

            // Tool selector
            let toolLabel = L.DomUtil.create('label', 'tilemarker-label', container);
            toolLabel.innerHTML = 'Tool';
            let toolSection = L.DomUtil.create('div', 'tilemarker-section tilemarker-toolbar', container);
            this._toolBtns = {};
            for (let tool of TOOLS) {
                let btn = L.DomUtil.create('button', 'tilemarker-btn tilemarker-tool-btn', toolSection);
            ...
            let colorSection = L.DomUtil.create('div', 'tilemarker-section tilemarker-color-section', container);
            let colorLabel = L.DomUtil.create('label', 'tilemarker-label', colorSection);
            ...
            let updateColor = () => {
                ...
                this._opacity = parseInt(this._opacityInput.value) / 100;
            ...
            // Counter
            let counterSection = L.DomUtil.create('div', 'tilemarker-section', container);
            this._counter = L.DomUtil.create('span', 'tilemarker-counter', counterSection);
            this._counter.innerHTML = '0 markers';

            // Import / Export / Clear
            let ioSection = L.DomUtil.create('div', 'tilemarker-section', container);

            let importBtn = L.DomUtil.create('button', 'tilemarker-btn', ioSection);
            importBtn.innerHTML = 'Import';
            importBtn.addEventListener('click', () => this._import());

            let exportBtn = L.DomUtil.create('button', 'tilemarker-btn', ioSection);
            exportBtn.innerHTML = 'Export';
            exportBtn.addEventListener('click', () => this._export());

            let clearBtn = L.DomUtil.create('button', 'tilemarker-btn tilemarker-btn-danger', ioSection);
        ...
        _isActive: function () {
            return this._container && this._container.contains(this.expanded);
        },
        ...
        _onMapClick: function (e) {
            ...
            if (this._tool === 'point') {
                let key = markerKey(plane, tile.x, tile.y);
                let existingIdx = this._markerIndex[key];
        ...
        _editMarker: function (idx, tileX, tileY) {
            let marker = this._markers[idx];
            // Parse the current ARGB color to get the CSS hex and alpha
            let hex = marker.color.replace('#', '');
            let currentAlpha = hex.length === 8 ? parseInt(hex.substring(0, 2), 16) : 255;
            let currentRgb = '#' + (hex.length === 8 ? hex.substring(2) : hex);
            let currentLabel = marker.label || '';
            ...
            .setContent(
                ...
                '    <input type="color" class="tilemarker-edit-color" value="' + currentRgb + '">' +
                '    <input type="range" class="tilemarker-edit-opacity" min="0" max="100" value="' + Math.round(currentAlpha / 255 * 100) + '">' +
                '    <span class="tilemarker-edit-opacity-val">' + Math.round(currentAlpha / 255 * 100) + '%</span>' +
                '  </div>' +
                '  <label class="tilemarker-edit-label">Label</label>' +
                '  <input type="text" class="tilemarker-edit-text" value="' + currentLabel.replace(/"/g, '&quot;') + '" placeholder="optional label" maxlength="255">' +
            ...
            let opacityInput = container.querySelector('.tilemarker-edit-opacity');
            let opacityVal = container.querySelector('.tilemarker-edit-opacity-val');
            opacityInput.addEventListener('input', () => {
                opacityVal.textContent = opacityInput.value + '%';
            });

            container.querySelector('.tilemarker-edit-save').addEventListener('click', () => {
                let newRgb = container.querySelector('.tilemarker-edit-color').value;
                let newOpacity = parseInt(opacityInput.value) / 100;
                let newColor = cssToArgb(newRgb, newOpacity);
                let newLabel = container.querySelector('.tilemarker-edit-text').value.trim() || undefined;

                marker.color = newColor;
                marker.label = newLabel;
                let cssColor = argbToCss(newColor);
            ...
            container.querySelector('.tilemarker-edit-delete').addEventListener('click', () => {
                let plane = marker.z;
                let coords = toGlobalCoords(marker.regionId, marker.regionX, marker.regionY);
                this._beginAction('remove');
        ...
        _showLinePreview: function (start, end) {
            ...
            this._preview = L.polyline(
                [[start.y + 0.5, start.x + 0.5], [end.y + 0.5, end.x + 0.5]],
        ...
        _commitRect: function (start, end) {
            let plane = this._map.getPlane();
            let tiles = rectTiles(start.x, start.y, end.x, end.y);
        ...
        _commitLine: function (start, end) {
            let plane = this._map.getPlane();
            let tiles = bresenhamLine(start.x, start.y, end.x, end.y);
            for (let [x, y] of tiles) {
                this._placeMarker(plane, x, y);
            }
        },
        ...
        _undo: function () {
            ...
            if (action.type === 'add') {
                ...
                let redoTiles = [];
                for (let t of action.tiles) {
                    let key = markerKey(t.plane, t.x, t.y);
                    let idx = this._markerIndex[key];
                    if (idx === undefined) continue;
                    let m = this._markers[idx];
        ...
        _placeMarker: function (plane, globalX, globalY) {
            let key = markerKey(plane, globalX, globalY);
        ...
        _rawPlace: function (plane, globalX, globalY, color, label) {
            ...
            let region = toRegionCoords(plane, globalX, globalY);
            let cssColor = argbToCss(color);

            let rectangle = L.rectangle(
                [[globalY, globalX], [globalY + 1, globalX + 1]],
        ...
        _rawRemove: function (key, idx) {
            ...
            let last = this._markers.length - 1;
            if (idx !== last) {
                let moved = this._markers[last];
                this._markers[idx] = moved;
                let movedCoords = toGlobalCoords(moved.regionId, moved.regionX, moved.regionY);
        ...
        _syncToUrl: function () {
            ...
            let packed = packMarkers(this._markers);
        ...
        _loadFromUrl: function () {
            let url = new URL(window.location.href);
            let encoded = url.searchParams.get('markers');
            if (!encoded) return;

            decompressFromUrl(encoded).then(bytes => {
                let data = unpackMarkers(bytes);
                for (let m of data) {
                    let coords = toGlobalCoords(m.regionId, m.regionX, m.regionY);
                    let key = markerKey(m.z, coords.globalX, coords.globalY);
                ...
                if (data.length > 0) {
                    let first = data[0];
                    let coords = toGlobalCoords(first.regionId, first.regionX, first.regionY);
        ...
        _import: function () {
            let input = prompt('Paste RuneLite Ground Markers JSON:');
            if (!input) return;
            ...
            try {
                ...
                for (let marker of data) {
                    ...
                    let plane = marker.z || 0;
                    let coords = toGlobalCoords(marker.regionId, marker.regionX, marker.regionY);
                    let key = markerKey(plane, coords.globalX, coords.globalY);
                    if (this._markerIndex[key] !== undefined) continue;
                    ...
                    if (typeof marker.color === 'string') {
                        ...
                        let intColor = marker.color.value >>> 0;
                        color = '#' + intColor.toString(16).padStart(8, '0').toUpperCase();
                    }

                    let cssColor = argbToCss(color);
                    let rectangle = L.rectangle(
                        [[coords.globalY, coords.globalX], [coords.globalY + 1, coords.globalX + 1]],
                        { color: cssColor, fillColor: cssColor, fillOpacity: 0.4, weight: 2, interactive: false }
                    ).addTo(this._map);
        ...
        expand: function () {
            ...
            let toolDef = TOOLS.find(t => t.id === this._tool);
    ...
    L.control.display.tileMarkers = function (options) {
        return new L.Control.Display.TileMarkers(options);
    };
