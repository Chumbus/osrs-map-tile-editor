...
L.GameMap = L.Map.extend({
	initialize: function (id, options) {
		...
		options.zoom = Number(
			parsedUrl.searchParams.get("zoom") || parsedUrl.searchParams.get("z") || this._limitZoom(options.zoom) || 0,
		);

		this._plane = Number(
			parsedUrl.searchParams.get("plane") ||
				parsedUrl.searchParams.get("p") ||
				this._limitPlane(options.plane) ||
				0,
		);

		this._mapId = Number(
			parsedUrl.searchParams.get("mapId") ||
				parsedUrl.searchParams.get("mapid") ||
				parsedUrl.searchParams.get("m") ||
				options.initialMapId ||
				-1,
		);
	...
	addMessage: function (message) {
		if (this.options.messageBox) {
			const messageBox = L.DomUtil.create("div", "leaflet-control-message-box");

			const messageContent = L.DomUtil.create("div", "leaflet-control-message-content");
			messageContent.innerHTML = message;
			messageBox.appendChild(messageContent);

			const clearButton = L.DomUtil.create("div", "leaflet-control-message-clear");
	...
	_validateMapId: function (_mapId) {
		const parsedMapId = parseInt(_mapId, 10);
	...
	setPlane: function (_plane) {
		const newPlane = this._limitPlane(_plane);
	...
	setMapId: function (_mapId) {
		const newMapId = this._validateMapId(_mapId);
	...
	updateAttribution: function (e) {
		const attr = this.attributionControl;
	...
	setEra: function (newEra, oldEra) {
		...
		const listeners = this._events.erachange;
		const event = {
			oldEra: oldEra,
			newEra: newEra,
		};
		...
		for (const listener of listeners) {
			const ready = listener.fn.call(listener.ctx || this, event);
			states.push(ready);
		}
...
L.gameMap = (id, options) => new L.GameMap(id, options);
...
L.TileLayer.Main = L.TileLayer.extend({
	...
	onAdd: function (map) {
		if (!this.options.errorTileUrl) {
			console.warn(
				`The ${this.options.source} layer did not have its errorTileUrl option set. This is needed to stop flickering.`,
			);
		}
	...
	refresh: async function (sentinel) {
		...
		for (const tile of Object.values(this._tiles)) {
			if (tile.current && tile.active !== false) {
				...
				const state = this._refreshTileUrl(this, tile, newsrc, sentinel, sentinel_ref);
...
L.grid = (options) => new L.Grid(options);
...
L.Heatmap = L.GridLayer.extend({
	...
	fetchData: function (npcNames, npcIds, range) {
		this.getIds(npcNames, npcIds).then((ids) => {
			Promise.allSettled(ids.map((id) => fetch(`${this.options.folder}/npcids/npcid=${id}.json`)))
				...
				.then((npcs) => {
					const keys = this.array.unique(npcs.flatMap((npc) => this.getRange(npc, range)));
	...
	constructDataCache: function (mapData, keys, npcs) {
		...
		if (this.options.showHeat) {
			const heat = keys.map((key) => this.createHeatmap(key));
	...
	constructNpcCache: function (_keys, npcs) {
	...
	createHeatmap: function (key) {
		const mapData = this._collisionData[key];

		const range = this.options.range;
		const npcs = this._npcData.filter((npc) => this.isInRange(key, npc, range));
		...
		const npcsHeat = npcs.map((npc) => {
			const npcHeat = this.array.zeros(64);
			const localNpcX = npc.x - 64 * i;
			const localNpcY = npc.y - 64 * j;
			const drawRange = {
		...
		const totalHeat = this.array.add(npcsHeat);
	...
	array: {
		...
		starMap: (array, fn) =>
			array.map((subarray, index, array) => subarray.map((value, jndex) => fn(value, index, jndex, array))),

		//similar to Python's itertools.combinations()
		combinations: (plane, array1, array2) => array1.flatMap((d) => array2.map((v) => [plane, d, v])),

		//runs function fn over a 2d array
		starEach: (array, fn) =>
			array.forEach((subarray, index, array) =>
				subarray.forEach((value, jndex) => fn(value, index, jndex, array)),
			),
	...
	getColor: function (tileData) {
		const key = tileData.toString();
		if (!this.colors[key]) {
			this.colors[key] =
				"rgba(" +
				parseInt((255 * tileData) / this._maxHeat, 10) +
				",0, 0, " +
				parseInt((100 * tileData) / this._maxHeat, 10) / 100 +
				")";
		}
	...
	getTextColor: function (tileData) {
		...
		if (!this.textColors[key]) {
			...
			this.textColors[key] = `rgba( 255 ,255, 255, ${parseInt((100 * tileData) / this._maxHeat, 10) / 100})`;
		}
		return this.textColors[key];
	},

	getIconUrl: function (npc) {
		const filename = `${this._npcIcons[npc.name]}.png`;
		if (filename) {
	...
	addMarker: function (npc, map) {
		...
		const marker = L.marker([npc.y + 0.5, npc.x + 0.5], {
		...
		const popUpText = Object.entries(npc)
	...
	getFeature: function (npc) {
		const key = this._generateDataKey(npc);

		npc.feature = this._collisionData[key]
			? this._collisionData[key][npc.x & (this.options.gridSize - 1)][npc.y & (this.options.gridSize - 1)].f
			: null;
	},

	getRange: function (npc, range) {
		const plane = npc.p;
		const radiusX = this.radius(npc.x, range);
		const radiusY = this.radius(npc.y, range);
		const allKeys = this.array.combinations(plane, radiusX, radiusY).map((tile) => this._generateDataKey(tile));

		return allKeys;
	},

	radius: function (center, radius) {
		const start = (center - radius) >> this.options.bitShift;
		const end = (center + radius) >> this.options.bitShift;
	...
	createTile: function (coords, done) {
		...
		const properX = coords.x >> (coords.z - 2);
		const properY = -(1 + coords.y) >> (coords.z - 2);

		var error;

		const key = this._generateDataKey(plane, properX, properY);
	...
	_drawTile: function (tile, coords, data) {
		const pixelsInGameTile = this.options.gameTilePx * 2 ** (coords.z - this.options.granularity);
		const gameTilesInTile = this.options.gridSize * 2 ** (this.options.granularity - coords.z);
		const modifier = 2 ** (coords.z - this.options.granularity) - 1;

		const startX = (coords.x & modifier) * gameTilesInTile;
		const startY = (-(1 + coords.y) & modifier) * gameTilesInTile;

		var ctx = tile.getContext("2d");

		for (let i = startX; i < startX + gameTilesInTile; i++) {
			for (let j = startY; j < startY + gameTilesInTile; j++) {
				const tileData = data[i][j];
				if (tileData) {
	...
	_drawRect: function (ctx, startX, startY, i, j, pixelsInGameTile, tileData) {
		...
		const x = (i - startX) * pixelsInGameTile;
		const y = this.getTileSize().y - (j + 1 - startY) * pixelsInGameTile;
...
L.heatmap = (options) => new L.Heatmap(options);
...
L.DynamicIcons = L.Layer.extend({
	...
	_update: function (center) {
		...
		var pixelBounds = this._getTiledPixelBounds(center),
			...
			_tileCenter = tileRange.getCenter(),
			queue = [],
	...
	getAverageLatLng: (icons) => {
		const latlngs = icons.map((icon) => icon.getLatLng());
		const lat = latlngs.map((latlng) => latlng.lat).reduce((a, b) => a + b, 0) / icons.length;
		const lng = latlngs.map((latlng) => latlng.lng).reduce((a, b) => a + b, 0) / icons.length;
	...
	createIcon: function (item) {
		...
		const marker = L.marker([item.y + 0.5, item.x + 0.5], {
		...
		const popUpText = Object.entries(item)
			.map((x) => x.map((i) => (typeof i !== "string" ? JSON.stringify(i) : i)).join(" = "))
			.join("<br>");
		marker.bindPopup(popUpText, {
			autoPan: false,
	...
	createPopupBody: function (mode, map, item) {
		...
		const nav =
			item.start && item.destination ? this.createNavigator(mode, map, item) : document.createElement("div");
...
L.dynamicIcons = (options) => new L.DynamicIcons(options);
...
L.Teleports = L.DynamicIcons.extend({
	...
	parseData: function (data, watery) {
		const dataCollection = this.parseSheet(data)
			.map(this.parseItems.bind(this))
			.flatMap((group) => group.items)
			.filter(Boolean)
			.map((item) => {
				item.plane = watery[item.x >> 6][item.y >> 6].includes((item.x << 14) + item.y)
					? item.plane - 1
					: item.plane;
				if ("destination" in item) {
					item.destination.plane = watery[item.destination.x >> 6][item.destination.y >> 6].includes(
						(item.destination.x << 14) + item.destination.y,
					)
						? item.destination.plane - 1
						: item.destination.plane;
				}
				if ("start" in item) {
					item.start.plane = watery[item.start.x >> 6][item.start.y >> 6].includes(
						(item.start.x << 14) + item.start.y,
					)
						? item.start.plane - 1
						: item.start.plane;
				}
		...
		const transits = dataCollection.filter((item) => "start" in item && "destination" in item);
		const transits_a = transits.map((item) =>
			Object.assign(
				{
					...item,
				},
		...
		const transits_b = transits.map((item) =>
			Object.assign(
				{
					...item,
				},
		...
		const teleports = dataCollection.filter(
			(item) => !("start" in item) && "destination" in item && !("type" in item),
		);
	...
	getIconUrl: (item) => {
		const filename = item.icon ? `${item.icon.trim()}.png` : undefined;
		if (filename) {
			var hash = MD5.md5(filename);
	...
	onAdd: function (_map) {
		...
		if (this.options.API_KEY && this.options.SHEET_ID) {
			const dataPromise = fetch(
				`https://sheets.googleapis.com/v4/spreadsheets/${this.options.SHEET_ID}/values/A:Z?key=${this.options.API_KEY}`,
			).then((response) =>
				response.ok
					...
					: response
							.json()
							.then((oopsie) =>
								Promise.reject(new Error(oopsie.error.message)).then(() => {}, console.error),
							),
			);

			const wateryPromise = fetch(
				`https://raw.githubusercontent.com/mejrs/data_rs3/refs/heads/master/keyed_watery.json`,
			)
				.then((response) =>
					response.ok
						? response.json()
						: Promise.reject(new Error(`${response.status} Error fetching ${response.url}`)),
				)
				.catch(console.error);
	...
	applyFanOut: (original_marker, marker, zoom) => {
		const bounds = original_marker.getLatLng();
		return (
			marker._item.type === "teleport" &&
			marker.fanned !== true &&
			marker.getLatLng().equals(bounds, 1.5 * 2 ** (4 - zoom))
		);
	},
	...
	fanOut: function (original_marker) {
		...
		const key = original_marker._item.key;
		const affectedIcons = this._icons[key].icons.filter((marker) =>
			this.applyFanOut(original_marker, marker, zoom),
		);

		const nSides = affectedIcons.length;
		if (nSides < 2) {
			return;
		}

		const radius = (2 ** (4 - zoom) * this.options.fanRadius) / (2 * Math.sin(Math.PI / nSides));

		const polygonPoints = Array.from(Array(nSides).keys(), (_x, index) => ({
			lng: radius * Math.sin((2 * index * Math.PI) / nSides),
			lat: radius * Math.cos((2 * index * Math.PI) / nSides),
		}));
		const polygonCenter = this.getAverageLatLng(affectedIcons);

		affectedIcons.forEach((marker, index) => this.fan(polygonCenter, marker, polygonPoints[index]));

		const eventFn = (e) => this.checkUnFan(e, polygonCenter, radius, eventFn, () => this.unFanAll(affectedIcons));
	...
	fanEvents: {
		...
		remove: function (obj, ev, fn) {
			const index = this.current.findIndex(
				(element) => element.obj === obj && element.ev === ev && element.fn === fn,
			);
			obj.off(ev, fn);
			if (index !== -1) {
	...
	fan: (polygonCenter, marker, transform) => {
	...
	createIcon: function (item) {
		...
		if (item.iconUrl) {
			const teleclass = item.type === "teleport" ? " teleport-icon" : "";
			var thisIcon = L.divIcon({
				html:
					'<img class="map-icon plane-' +
					item.plane +
					teleclass +
					'" src="' +
					item.iconUrl +
		...
		const popUpBody = this.createPopupBody(item.mode, this._map, item);
		...
		if ("start" in item && "destination" in item) {
			destinationMarker.on("mouseover", function () {
				...
				const travel = L.polyline(points, {
					color: "white",
				});
	...
	detectNewHeader: (row, previousRow) => {
	...
	parseSheet: function (sheet) {
		...
		sheet.forEach((row, rowNumber, array) => {
			const previousRow = array[rowNumber - 1] ?? [];
	...
	parseItems: function (group) {
		...
		group.items = group.items.map((item) => {
			const endPos = item["Pos (End)"] ?? item.Pos;
			const endLook = item["Look (End)"] ?? item.Look;
			if (!endPos || !endLook || endPos === "-" || endLook === "-") {
				return;
			}
			const destination = this.parseCoord(item, endPos, endLook);
			item.destination = destination;

			const startPos = item["Pos (Start)"] ?? item.Pos;
			const startLook = item["Look (Start)"] ?? item.Look;
			if (
				startPos &&
				startLook &&
				startPos !== "-" &&
				...
				const start = this.parseCoord(item, startPos, startLook);
				item.start = start;
			}
	...
	parseCoord: (item, pos, look) => {
		const _plane = Number(pos);
...
L.teleports = (options) => new L.Teleports(options);
		...
		if (this.options.API_KEY && this.options.SHEET_ID) {
			const dataPromise = fetch(
				...
				response.ok
					...
					: response
							.json()
							.then((e) => Promise.reject(new Error(e.error.message)).then(() => {}, console.error)),
			);
...
L.customParseTeleports = (options) => new L.CustomParseTeleports(options);

const _rect = L.DivIcon.extend({
	options: {
...
L.CrowdSourceMovement = L.DynamicIcons.extend({
	...
	createTeleportIcon: function (item) {
		const teleportMarker = L.marker([item.y + 0.5, item.x + 0.5]);
		...
		if ("destinations" in item) {
			teleportMarker.on("mouseover", () => {
				item.destinations.forEach((destination) => {
					...
					const destmarker = L.marker([destination.y + 0.5, destination.x + 0.5], {
						icon: new VertexIcon(),
					}).addTo(this._map);
		...
		const popUp = this.createPopup(item.mode, this._map, item);
	...
	createTransportIcon: function (item) {
		...
		const startMarker = L.marker([item.y + 0.5, item.x + 0.5], {
			icon: icon,
		});
	...
	getIconUrl: (name) => {
		const hash = MD5.md5(`${name}.png`);
		const iconUrl = `https://runescape.wiki/images/${hash.substr(0, 1)}/${hash.substr(0, 2)}/${name}.png`;
		return iconUrl;
	},
	...
	createPopup: function (mode, map, item) {
		...
		const nav =
			item.from_coordinate && item.to_coordinate
				? this.createNavigator(mode, map, item)
	...
	createNavigator: (_mode, map, item) => {
		const newButton = document.createElement("button");
		newButton.innerHTML = "Navigate to link";
		newButton.onclick = () => {
...
L.crowdSourceMovement = (options) => new L.CrowdSourceMovement(options);
...
L.Varbit = L.DynamicIcons.extend({
	...
	parseData: function (data) {
		const linear_data = data.map((item) => Object.assign(item, item.location));
		...
		const reallyLoadEverything =
			linear_data.length < 10000 ? true : confirm(`Really load ${linear_data.length} markers?`);
...
L.varbit = (options) => new L.Varbit(options);
